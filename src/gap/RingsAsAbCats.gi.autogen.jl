...
@InstallMethod( RING_AS_CATEGORY,
  ...
  function( CAP_NAMED_ARGUMENTS, ring )
    ...
    category = CreateCapCategory( @Concatenation( "RingAsCategory( ", StringGAP( ring )," )" ), IsRingAsCategory, IsRingAsCategoryObject, IsRingAsCategoryMorphism, IsCapCategoryTwoCell );
...
##
@InstallMethod( RingAsCategoryMorphism,
...
@InstallGlobalFunction( INSTALL_FUNCTIONS_FOR_RING_AS_CATEGORY,
  ...
  function( category )
    ...
    #= comment for Julia
    if (IsHomalgRing( ring ) && HasIsExteriorRing( ring ) && IsExteriorRing( ring ) && IsField( BaseRing( ring ) ))
        ...
        for k in (1):(l)
            for comb in Combinations( indets, k )
