...
@InstallGlobalFunction( FREYD_CATEGORY,
  ...
  function( CAP_NAMED_ARGUMENTS, underlying_category )
    ...
    if (@IsBound( underlying_category.supports_empty_limits ))
...
@InstallMethod( FREYD_CATEGORIES_SimplifyObjectTupleOp,
  ...
  function( object, i )
    ...
    while true
      ...
      if (counter >= i || IsIsomorphism( red_from_last_step ))
  ...
  function( category )
    ...
    is_possible_to_install = function( to_be_installed, to_be_tested )
        ...
        Perform( to_be_tested, function(x)
            if (@not CanCompute( underlying_category, x ))
                Add( not_supported, x );
            end;
        end );
        
        # methods cannot be installed, so inform the user
        if (@not IsEmpty( not_supported ))
    ...
    AddMorphismConstructor( category,
      function( cat, source, morphism_datum, range )
        ...
        if (@not IsEqualForObjects( UnderlyingCategory( cat ), Source( morphism_datum ), Range( ObjectDatum( cat, source ) ) ))
        ...
        if (@not IsEqualForObjects( UnderlyingCategory( cat ), Range( morphism_datum ), Range( ObjectDatum( cat, range ) ) ))
    ...
    AddIsWellDefinedForObjects( category,
      function( cat, object )
        ...
        if (@not IsWellDefinedForMorphisms( underlying_category, ObjectDatum( cat, object ) ))
    ...
    AddIsWellDefinedForMorphisms( category,
      function( cat, morphism )
        ...
        if (@not IsWellDefinedForMorphisms( underlying_category, MorphismDatum( cat, morphism ) ))
            
            return false;
            
        elseif (@not IsLiftable( underlying_category, PreCompose( underlying_category, ObjectDatum( cat, Source( morphism ) ), MorphismDatum( cat, morphism ) ), ObjectDatum( cat, Range( morphism ) ) ))
    ...
    if (is_possible_to_install( "Homomorphism structure",
        ...
        if (HasIsAbelianCategory( underlying_range_category )
            ...
            && (HasIsProjective( distinguished_object ) || (CanCompute( underlying_range_category, "IsProjective" ) && OperationWeight( underlying_range_category, "IsProjective" ) <= 50))
        ...
        elseif (HasIsAdditiveCategory( underlying_range_category )
                && IsAdditiveCategory( underlying_range_category )
                && ForAll(
                    [ "ProjectionOfBiasedWeakFiberProduct", "UniversalMorphismIntoBiasedWeakFiberProduct" ],
                    f -> CanCompute( underlying_range_category, f ) ) )
            
            if (IsIdenticalObj( underlying_range_category, underlying_category ))
        ...
        if (@not homomorphism_structure_derivation_case == "none")
    ...
    if (is_possible_to_install( "TensorProductOnObjects",
                               [ "TensorProductOnObjects", "TensorProductOnMorphismsWithGivenTensorProducts" ] ))
    ...
    if (is_possible_to_install( "TensorUnit", [ "TensorUnit" ] ))
    ...
    if (is_possible_to_install( "AssociatorLeftToRightWithGivenTensorProducts", [ "AssociatorLeftToRightWithGivenTensorProducts" ] ))
    ...
    # Note that even if the underlying category is a strict monoidal category
    # (i.e. the associators and unitors are identities), this need not be true in the Freyd category.
    if (is_possible_to_install( "AssociatorRightToLeftWithGivenTensorProducts", [ "AssociatorRightToLeftWithGivenTensorProducts" ] ))
    ...
    if (is_possible_to_install( "LeftUnitorWithGivenTensorProduct", [ "LeftUnitorWithGivenTensorProduct" ] ))
    ...
    if (is_possible_to_install( "LeftUnitorInverseWithGivenTensorProduct", [ "LeftUnitorInverseWithGivenTensorProduct" ] ))
    ...
    # We derive from the underlying category.
    if (is_possible_to_install( "RightUnitorWithGivenTensorProduct", [ "RightUnitorWithGivenTensorProduct" ] ))
    ...
    if (is_possible_to_install( "RightUnitorInverseWithGivenTensorProduct", [ "RightUnitorInverseWithGivenTensorProduct" ] ))
    ...
    if (is_possible_to_install( "BraidingWithGivenTensorProducts", [ "BraidingWithGivenTensorProducts" ] ))
    ...
    if (is_possible_to_install( "InternalHomOnObjects", 
                               [ "InternalHomOnMorphismsWithGivenInternalHoms", "ProjectionOfBiasedWeakFiberProduct", "UniversalMorphismIntoBiasedWeakFiberProduct" ] ))
    ...
    if (is_possible_to_install( "ClosedMonoidalLeftEvaluationMorphismWithGivenSource",
                                [ "InternalHomOnMorphismsWithGivenInternalHoms", "ProjectionOfBiasedWeakFiberProduct", 
                                  ...
                                  "TensorProductOnMorphismsWithGivenTensorProducts", "ClosedMonoidalLeftEvaluationMorphismWithGivenSource" ] ))
        ...
        AddClosedMonoidalLeftEvaluationMorphismWithGivenSource( category,
            function( cat, A, B, S )
                ...
                eval_concat = PreCompose( cat, AsFreydCategoryMorphism( cat, ClosedMonoidalLeftEvaluationMorphism( underlying_category, a, b ) ), proj_B );
    ...
    if (is_possible_to_install( "ClosedMonoidalLeftCoevaluationMorphismWithGivenRange",
                                [ "InternalHomOnMorphismsWithGivenInternalHoms", "ProjectionOfBiasedWeakFiberProduct", 
                                  "UniversalMorphismIntoBiasedWeakFiberProduct", "TensorProductOnObjects",
                                  "TensorProductOnMorphismsWithGivenTensorProducts", "ClosedMonoidalLeftCoevaluationMorphismWithGivenRange" ] ))
    ...
    if (ForAll( [ "RandomObjectByList",
                 ...
                 "ProjectionInSecondFactorOfWeakBiFiberProduct" ], m -> CanCompute( underlying_category, m ) ))
      ...
      if (CanCompute( underlying_category, "IsZeroForObjects" ) &&
          CanCompute( underlying_category, "RandomMorphismWithFixedSourceAndRangeByList" ) &&
            HasRangeCategoryOfHomomorphismStructure( category ) &&
              ( IsIdenticalObj( RangeCategoryOfHomomorphismStructure( category ), category ) ||
                IsIdenticalObj( RangeCategoryOfHomomorphismStructure( category ), underlying_category ) ))
        ...
        AddRandomMorphismWithFixedSourceAndRangeByList( category,
          function ( cat, S, R, L )
            ...
            if (IsZeroForObjects( underlying_category, Source( RelationMorphism( S ) ) ))
              ...
              # If the category has homomorphism structure over itself)
    ...
    if (ForAll( [ "RandomObjectByInteger",
      ...
      if (CanCompute( underlying_category, "IsZeroForObjects" ) &&
          CanCompute( underlying_category, "RandomMorphismWithFixedSourceAndRangeByInteger" ) &&
            HasRangeCategoryOfHomomorphismStructure( category ) &&
              ( IsIdenticalObj( RangeCategoryOfHomomorphismStructure( category ), category ) ||
                IsIdenticalObj( RangeCategoryOfHomomorphismStructure( category ), underlying_category ) ))
...
@InstallGlobalFunction( IsValidInputForFreydCategory,
  function( category )
    ...
    for i in required_ops

      if (@not CanCompute( category, i ))
    ...
    if (@not HasIsAdditiveCategory( category ))
...
@InstallMethod( /,
  ...
  function( obj, freyd_category )
    ...
    if (@not IsIdenticalObj( UnderlyingCategory( freyd_category ), CapCategory( obj ) ))
