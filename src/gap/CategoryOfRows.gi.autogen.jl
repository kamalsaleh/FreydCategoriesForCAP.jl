...
include( "precompiled_categories/CategoryOfRows_as_AdditiveClosure_RingAsCategory_Field_precompiled.gi.autogen.jl" );
include( "precompiled_categories/CategoryOfRows_as_AdditiveClosure_RingAsCategory_CommutativeRing_precompiled.gi.autogen.jl" );
include( "precompiled_categories/CategoryOfRows_as_AdditiveClosure_RingAsCategory_HomalgExteriorRingOverField_precompiled.gi.autogen.jl" );
include( "precompiled_categories/CategoryOfRows_as_AdditiveClosure_RingAsCategory_ArbitraryRing_precompiled.gi.autogen.jl" );
...
@InstallMethod( CategoryOfRowsMorphism,
  ...
  function( cat, source, homalg_matrix, range )
    ...
    if (NrRows( homalg_matrix ) != RankOfObject( source ))
        
        Error( "the number of rows has to be equal to the rank of the source" );
        
    end;
    
    if (NrColumns( homalg_matrix ) != RankOfObject( range ))
...
@InstallMethod( CATEGORY_OF_ROWS_ReductionBySplitEpiSummandTuple,
  ...
  function( alpha )
    ...
    if (NrColumns( S ) == RankOfObject( Range( alpha ) ))
...
@InstallGlobalFunction( INSTALL_FUNCTIONS_FOR_CATEGORY_OF_ROWS,
  ...
  function( category )
    ...
    is_defined_over_field = HasIsFieldForHomalg( ring ) && IsFieldForHomalg( ring );
    ...
    AddIsWellDefinedForObjects( category,
      function( cat, object )
        ...
        if (@not IsInt( RankOfObject( object ) ))
            
            return false;
            
        elseif (!(RankOfObject( object ) >= 0))
    ...
    AddIsWellDefinedForMorphisms( category,
      function( cat, morphism )
        ...
        if (@not IsHomalgMatrix( UnderlyingMatrix( morphism ) ))
            
            return false;
            
        elseif (!(NrRows( UnderlyingMatrix( morphism ) ) == RankOfObject( Source( morphism ) )))
            
            return false;
            
        elseif (!(NrColumns( UnderlyingMatrix( morphism ) ) == RankOfObject( Range( morphism ) )))
    ...
    AddRandomObjectByList( category,
      function ( category, L )
        ...
        if (IsEmpty( L ) || ForAny( L, IsNegInt ))
    ...
    AddRandomMorphismWithFixedSourceAndRangeByList( category,
      function ( category, S, R, L )
        ...
        if (@not ForAll( L, c -> ForAny( [ IsInt, IsHomalgRingElement ], is -> is( c ) ) ))
...
@InstallMethod( StringGAP,
  ...
  function( category_of_rows_object )
    ...
    return @Concatenation( "A row module over ",
                          ...
                          " of rank ", StringGAP( RankOfObject( category_of_rows_object ) ) );
...
@InstallMethod( ViewString,
  ...
  function( category_of_rows_object )
    ...
    return @Concatenation( "<", StringGAP( category_of_rows_object ), ">" );
