  ...
  function( category, checks )
    ...
    AddIsWellDefinedForObjects( category,
      function( cat, object )
        ...
        if (Rank( object ) == 0)
          ...
          for i in (1):(Length( DegreeList( object ) ))
          
            if (@not IsHomalgModuleElement( DegreeList( object )[ i ][ 1 ] ))
            
              # the degrees are not saves as homalg module elements, so return false
              return false;
            
            elseif (@not IsIdenticalObj( SuperObject( DegreeList( object )[ i ][ 1 ] ), A ))
    ...
    AddIsWellDefinedForMorphisms( category,
      function( cat, morphism )
        ...
        if (!( IsIdenticalObj( UnderlyingHomalgGradedRing( source ), UnderlyingHomalgGradedRing( morphism ) ) &&
                        IsIdenticalObj( UnderlyingHomalgGradedRing( morphism ), UnderlyingHomalgGradedRing( range ) ) ))
        ...
        if (!( Rank( source ) == NrColumns( morphism_matrix )
                 && NrRows( morphism_matrix ) == Rank( range ) ))
        ...
        source_degrees = [];
        for i in (1):(Length( DegreeList( source ) ))
        ...
        range_degrees = [];
        for i in (1):(Length( DegreeList( range ) ))
        ...
        for i in (1):(Rank( range ))
        
          # initialise the i-th buffer row
          buffer_row = List( (1):(Rank( source )) );

          # compute its entries
          for j in (1):(Rank( source ))
        
            if (morphism_matrix_entries[ i ][ j ] == Zero( HomalgRing( morphism_matrix ) ))
          ...
          if (Length( DuplicateFreeList( buffer_row ) ) > 1)
        ...
        if (!( range_degrees == dummy_range_degrees ))
    ...
    if (HasIsCommutative( UnderlyingNonGradedRing( UnderlyingGradedRing( category ) ) )
       && IsCommutative( UnderlyingNonGradedRing( UnderlyingGradedRing( category ) ) ))
    ...
    AddTensorProductOnObjects( category,
      function( cat, object1, object2 )
        ...
        if (IsZero( object1 ) || IsZero( object2 ))
