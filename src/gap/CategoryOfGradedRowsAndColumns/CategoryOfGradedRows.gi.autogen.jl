  ...
  function( category, checks )
    ...
    AddIsWellDefinedForMorphisms( category,
      function( cat, morphism )
        ...
        if (!( Rank( source ) == NrRows( morphism_matrix )
                 && NrColumns( morphism_matrix ) == Rank( range ) ))
        ...
        for i in (1):(Rank( range ))
          ...
          for j in (1):(Rank( source ))
            ...
            if (morphism_matrix_entries[ j ][ i ] == Zero( HomalgRing( morphism_matrix ) ))
          ...
          if (Length( DuplicateFreeList( buffer_col ) ) > 1)
